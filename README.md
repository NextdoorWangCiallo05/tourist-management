...
| **🤖 AI 助手** | 自然语言查询 / 异常检测 / 智能客服 / 参加者查询 | Function Calling 架构 + 规则引擎 + 可选 GPT-4o 集成 |
| **🌙 深色模式** | 一键切换 / CSS 变量驱动 | Element Plus 全组件适配 + AI 面板适配 |
| **🌐 多语言** | 中文 / English | vue-i18n 全组件覆盖 + AI 提示语中英自适应 |
...
系统内置 `POST /api/agent/chat` 与 `POST /api/agent/chat/stream` 接口（后者支持 SSE 流式输出），采用 **Function Calling 架构** + **增强 NLU 规则引擎** 实现开箱即用的智能对话。

### 架构设计

```
用户输入 → 增强 NLU 规则引擎（正则意图识别）
          ├── 命中 → 调用已注册 Tool → 格式化回复 → 返回
          ├── 未命中 → Function Calling（需 API Key）
          │            ├── GPT 选择 Tool + 提取参数 → 调用 → 格式化回复
          │            └── GPT 直接回答 → 返回
          └── 完全未命中 → Fallback 帮助提示 → 返回
```

所有工具函数通过 `@register_tool(name, schema)` 装饰器注册，自带参数描述，GPT 可自动选择并提取参数。

### 已注册工具

| 工具 | 参数 | 用途 |
|------|------|------|
| `search_groups` | keyword, date_from, date_to | 按关键词/日期搜索可报名旅游团 |
| `get_application` | application_no | 查询申请详情（含订金/余款状态） |
| `search_applications` | name | 按责任人姓名搜索申请 |
| `get_stats` | — | 系统统计数据概览 |
| `calculate_fee` | days_before, total_amount | 计算取消手续费 |
| `check_anomalies` | — | 检测系统异常（订金超期/出发过期/名额将满） |
| `get_participants` | application_no | 查询申请的参加者列表 |
...
| "取消手续费怎么算" | 显示完整手续费规则 |
| "参加者 AP12345678" | 查询申请的所有参加者 |

### 模式二：GPT Function Calling（需 API Key）

```bash
$env:OPENAI_API_KEY = "sk-xxx"
```

规则引擎未命中时，GPT-4o-mini 自动选择合适的 Tool 并提取参数，实现精准查询。

### 前端特性

| 特性 | 说明 |
|------|------|
| **深色模式适配** | AI 面板背景/气泡/输入框随 CSS 变量切换 |
| **消息持久化** | localStorage 保存最近 50 条，刷新不丢失 |
| **Markdown 渲染** | 支持 `**粗体**`、`` `代码` ``、`\| 表格 \|`、`· 列表` |
| **英文自适应** | 快捷提示/欢迎语/状态文字随 `$t()` 切换语言 |
| **多轮上下文** | 发送最近 10 条对话给后端，GPT 模式保持连贯 |
| **重新提问** | 每条 AI 回复带"重新提问"按钮，自动回填问题 |
| **SSE 流式输出** | `POST /api/agent/chat/stream` 支持 GPT 打字机效果 |
...
| 方法 | 路径 | 说明 |
|------|------|------|
| POST | `/api/agent/chat` | AI 对话（规则引擎 / Function Calling / GPT 回退） |
| POST | `/api/agent/chat/stream` | SSE 流式对话（GPT 模式逐字输出） |
...
| **AI 智能助手** | 规则引擎 + Function Calling 架构，7 个注册 Tool，GPT-4o-mini 可选，SSE 流式输出 |
